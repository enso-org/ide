...
where
    for<'t> &'t Self: IntoOwned<Owned = Self>,
{
    /// Translate the shape by a given offset.
    fn translate<V: Into<Var<Vector2<Pixels>>>>(
        &self,
        v: V,
    ) -> Translate<Self> {
        Translate(self, v)
    ...
    where
        (X, Var<Pixels>): Into<Var<Vector2<Pixels>>>,
    {
    ...
    where
        (Var<Pixels>, Y): Into<Var<Vector2<Pixels>>>,
    {
    ...
    fn intersection<S: IntoOwned>(
        &self,
        that: S,
    ) -> Intersection<Self, Owned<S>> {
