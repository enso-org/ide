...
impl pass::Definition for SymbolsRenderPass {
    fn initialize(&mut self, instance: &pass::Instance) {
        ...
        let out_id =
            pass::OutputDefinition::new("id", rgba, tex_type, id_params);
    ...
    fn run(&mut self, instance: &pass::Instance) {
        ...
        instance
            .context
            .clear_bufferfv_with_f32_array(Context::COLOR, 0, &arr);
        instance
            .context
            .clear_bufferfv_with_f32_array(Context::COLOR, 1, &arr);

        let mut scissor_stack = default();
        self.render_layer(
            instance,
            &self.layers.root.clone(),
            &mut scissor_stack,
            false,
        );
        ...
        instance
            .context
            .bind_framebuffer(Context::FRAMEBUFFER, None);
    }
}
...
impl SymbolsRenderPass {
    ...
    fn render_layer(
        ...
        let first_scissor_usage =
            scissor_box_changed && parent_scissor_box.is_none();
        if let Some(scissor_box) = scissor_box {
            if scissor_box_changed {
                ...
                instance
                    .context
                    .scissor(position.x, position.y, size.x, size.y);
            }
        }
        ...
        if nested_masking {
            ...
            let arr = vec![0.0, 0.0, 0.0, 0.0];
            instance.context.clear_bufferfv_with_f32_array(
                Context::COLOR,
                0,
                &arr,
            );
