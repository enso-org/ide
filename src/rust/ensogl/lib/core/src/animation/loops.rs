    ...
    Callback: RawLoopCallback,
{
    ...
    pub fn new(callback: Callback) -> Self {
        ...
        let on_frame = move |time| {
            weak_data.upgrade().for_each(|t| t.borrow_mut().run(time))
        };
        data.borrow_mut().on_frame = Some(Closure::new(on_frame));
        let handle_id = web::request_animation_frame(
    ...
    Callback: LoopCallback,
{
    ...
    pub fn new(callback: Callback) -> Self {
        ...
        let animation_loop =
            RawLoop::new(on_frame(callback, time_info.clone_ref()));
        Self {
            animation_loop,
            time_info,
        }
    ...
    Callback: LoopCallback,
{
    move |current_time: f32| {
        ...
        let start = if time_info.start == 0.0 {
            current_time
        } else {
            time_info.start
        };
...
impl<Callback: FnOnce<(TimeInfo,)>> FnOnce<(TimeInfo,)>
    for FixedFrameRateSampler<Callback>
{
    type Output = ();
...
impl<Callback: FnMut<(TimeInfo,)>> FnMut<(TimeInfo,)>
    for FixedFrameRateSampler<Callback>
{
...
impl DynamicLoop {
    ...
    pub fn on_frame<F: DynamicLoopCallback>(
        &self,
        callback: F,
    ) -> callback::Handle {
    ...
    pub fn on_before_frame<F: DynamicLoopCallback>(
        &self,
        callback: F,
    ) -> callback::Handle {
        self.data
    ...
    pub fn on_after_frame<F: DynamicLoopCallback>(
        &self,
        callback: F,
    ) -> callback::Handle {
        self.data
