    ...
    E::Model: Default,
{
    ...
    pub fn new(parent: impl AnyLogger, app: &Application) -> Self {
        ...
        let label_layer =
            Rc::new(Cell::new(app.display.scene().layers.label.id()));
        List {
            logger,
            app,
            display_object,
        ...
        if range != self.entries_range.get() {
            ...
            let current_entries: HashSet<entry::Id> =
                with(self.entries.borrow_mut(), |mut entries| {
                    entries
                        .resize_with(range.len(), || self.create_new_entry());
                    entries.iter().filter_map(|entry| entry.id.get()).collect()
                });
            let missing =
                range.clone().filter(|id| !current_entries.contains(id));
            ...
            with(self.entries.borrow(), |entries| {
                let is_outdated = |e: &DisplayedEntry<E>| {
                    e.id.get().map_or(true, |i| !range.contains(&i))
                };
    ...
    pub fn set_label_layer(&self, label_layer: LayerId) {
        if let Some(layer) = self
            ...
            .scene()
            .layers
            .get_sublayer(self.label_layer.get())
        {
    ...
    fn update_entry(
        ...
        match model {
            ...
            None => {
                error!(
                    logger,
                    "Model provider didn't return model for id {id}."
                );
