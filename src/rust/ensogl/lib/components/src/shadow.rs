...
pub fn parameters_from_style_path(
    style: &StyleWatch,
    path: impl Into<style::Path>,
) -> Parameters {
...
pub fn from_shape_with_parameters(
    base_shape: AnyShape,
...
pub fn from_shape_with_parameters_and_alpha(
    ...
    let shadow =
        shadow.translate((parameters.offset_x.px(), parameters.offset_y.px()));
...
pub fn add_to_dom_element(
    ...
    let shadow = format!(
        ...
        off_x, off_y, blur, spread, alpha
    );
    element
        .dom()
        .set_style_or_warn("box-shadow", shadow, logger);
}
