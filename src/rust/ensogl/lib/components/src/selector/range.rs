...
impl component::Frp<Model> for Frp {
    fn init(&self, app:&Application, model:&Model, style:&StyleWatchFrp){
        ...
        frp::extend! { network
            ...
            // Intern State Vales
            norm_value                 <- all2(&frp.value,&frp.bounds).map(|((lower,upper),bounds)|{
                (normalise_value(&(*lower,*bounds)),normalise_value(&(*upper,*bounds)))
            });
            ...
            has_underflow <- norm_value.map(|value| (value.0.min(value.1)) < 0.0);
            has_overflow  <- norm_value.map(|value| (value.0.max(value.1)) > 1.0);
            ...
            eval frp.value((value) {
                model.set_left_label_content(value.0);
                model.set_right_label_content(value.1);
            });

            // Mouse IO
            let change_left_and_right_value = base_frp.is_dragging_track.clone_ref();
            change_left_value <- base_frp.is_dragging_left_handle || base_frp.is_dragging_left_overflow;
            change_right_value <- base_frp.is_dragging_right_handle || base_frp.is_dragging_right_overflow;

            drag_movement   <- mouse.translation.gate(&base_frp.is_dragging_any);
            drag_delta      <- drag_movement.map2(&base_frp.track_max_width, |delta,width| (delta.x + delta.y) / width);

            drag_center_delta <- drag_delta.gate(&change_left_and_right_value);
            center_update     <- drag_center_delta.map2(&norm_value,|delta,(l,u)| (l+delta,u+delta));

            drag_left_delta <- drag_delta.gate(&change_left_value);
            left_update     <- drag_left_delta.map2(&norm_value,|delta,(l,u)| (l+delta,*u));

            drag_right_delta <- drag_delta.gate(&change_right_value);
            right_update     <- drag_right_delta.map2(&norm_value,|delta,(l,u)| (*l,u+delta));

            any_update   <- any3(&center_update,&left_update,&right_update);
            is_in_bounds <- any_update.map2(&normalised_overflow_bounds, |range,bounds| should_clamp_with_overflow(range,bounds));
            new_value_absolute <- all(&frp.set_bounds,&any_update).map(|(bounds,(lower,upper))|{
                sorted_range((absolute_value(&(*bounds,*lower)),absolute_value(&(*bounds,*upper))))
            });
