...
pub struct Map {
    children : HashMap<String,MapNode>,
}
...
impl Map {
    pub fn visualize(&self, name:&str, dot:&mut String) {
        writeln!(dot,"{}",name);
        for (path,map) in &self.children {
            writeln!(dot,"{} -> sheet_{} [label=\"{}\"]",name,map.sheet_id,path);
            map.visualize(dot);
        }
    }
...
impl MapNode {
    pub fn new(sheet_id:SheetId) -> Self {
        let var_id = default();
        let map    = default();
        Self {var_id,sheet_id,map}
    }

    pub fn visualize(&self, dot:&mut String) {
        let name = format!("sheet_{}",self.sheet_id);
        self.map.visualize(&name,dot);
    }
}
...
//    css.set_expr(&["text","size"],&[&["size"]],|t| t[0]);
    ...
    fn get_nested_sheet_id(&mut self, path_head:&str, path_tail:&[&str]) -> SheetId {
        ...
        let mut map      = Self::insert_sub_map_if_missing(sheets,&mut self.map.children,path_head);
        ...
        loop {
            match sub_path {
                ...
                [head, tail@..] => {
                    sub_path = tail;
                    map      = Self::insert_sub_map_if_missing(sheets,&mut map.map.children,*head);
                }
            }
        }
    }
    ...
    (sheets:&mut OptVec<Sheet>, map:&'t mut HashMap<String,MapNode>, item:&str) -> &'t mut MapNode {
        let missing = map.get(item).is_none();
        if missing {
            let sheet_id = sheets.insert_with_ix(|ix| Sheet::new(ix.into())).into();
            map.insert(item.into(), MapNode::new(sheet_id));
        }
    ...
    pub fn vizualize(&self) -> String {
        ...
        self.map.visualize("root",&mut dot);
