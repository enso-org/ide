...
pub fn entry_point_transformation_3d() {
    ...
    let sprite_square = sprite_system.new_instance();
    sprite_square.size.set(Vector2::new(110.0, 110.0));
    sprite_square.mod_position(|t| *t = Vector3::new(0.0, 75.0, 0.0));
    ...
    world.on_frame(move |time| {
        ...
        let _keep_alive = &sprite_square;
        let _keep_alive = &dom_square;
        let _keep_alive = &network;

        sprite_square.set_rotation(Vector3(0.0, time.local/1000.0, 0.0));
