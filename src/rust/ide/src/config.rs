...
#[derive(Clone, Copy, Debug, Fail)]
#[fail(
    display = "Provided options for both project manager and language server connection."
)]
...
impl Default for BackendService {
    fn default() -> Self {
        Self::ProjectManager {
            endpoint: constants::PROJECT_MANAGER_ENDPOINT.into(),
        }
    }
}
...
impl BackendService {
    ...
    pub fn from_web_arguments(args: &Args) -> FallibleResult<Self> {
        if let Some(endpoint) = &args.project_manager {
            if args.language_server_rpc.is_some()
                || args.language_server_data.is_some()
            {
            ...
            match (&args.language_server_rpc, &args.language_server_data) {
                (Some(json_endpoint), Some(binary_endpoint)) => {
                    ...
                    let namespace =
                        args.namespace.clone().unwrap_or_else(|| {
                            constants::DEFAULT_PROJECT_NAMESPACE.to_owned()
                        });
                ...
                (None, _) => {
                    Err(MissingOption(args.names().language_server_rpc())
                        .into())
                }
                (_, None) => {
                    Err(MissingOption(args.names().language_server_data())
                        .into())
                }
            }
        }
    ...
    fn default() -> Self {
        Self {
            ...
            project_name: ProjectName(
                constants::DEFAULT_PROJECT_NAME.to_owned(),
            ),
        }
    }
}
...
impl Startup {
    ...
    pub fn from_web_arguments() -> FallibleResult<Startup> {
        ...
        let project_name =
            ARGS.project.clone().map(|t| t.into()).unwrap_or_else(|| {
