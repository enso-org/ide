...
impl Suggestion {
    ...
    fn with_this_arg(
        mut self,
        this_arg: impl TryInto<tp::QualifiedName, Error: Debug>,
    ) -> Self {
        ...
        Iter::Item: TryInto<tp::QualifiedName, Error: Debug>,
    {
        ...
        let result =
            conv_results.collect::<Result<Vec<tp::QualifiedName>, _>>();
...
pub fn add_hardcoded_entries_to_list(
    ...
    SUGGESTIONS.with(|hardcoded| {
        for hc_root_category in hardcoded {
            ...
            let mut root_cat =
                list.add_root_category(hc_root_category.name, icon);
            for hc_category in &hc_root_category.categories {
                let icon = hc_root_category.icon.clone_ref();
                let category = root_cat.add_category(hc_category.name, icon);
                category.extend(
                    hc_category.suggestions.iter().cloned().filter_map(
                        |suggestion| {
                            let this_type_matches =
                            ...
                            let return_type_matches =
                                if let Some(return_types) = return_types {
                                    suggestion
                                        ...
                                        .as_ref()
                                        .map_or(false, |rt| {
                                            return_types.contains(rt)
                                        })
                                } else {
                                    true
                                };
                            let filtered_in =
                                this_type_matches && return_type_matches;
                            filtered_in.as_some_from(|| {
                                action::Action::Suggestion(
                                    action::Suggestion::Hardcoded(suggestion),
                                )
                            })
                        },
                    ),
