...
#[derive(Debug, Fail)]
#[fail(
    display = "An action \"{}\" is not supported: {}",
    action_label, reason
)]
...
#[derive(Copy, Clone, Debug, Fail)]
#[fail(
    display = "An action cannot be executed when searcher is in \"edit node\" mode."
)]
...
impl ParsedInput {
    ...
    fn next_completion_id(&self) -> CompletedFragmentId {
        match &self.expression {
            ...
            Some(expression) => CompletedFragmentId::Argument {
                index: expression.args.len(),
            },
        }
    }

    /// Get the picked fragment from the Searcher's input.
    pub fn completed_fragment(
        &self,
        fragment: CompletedFragmentId,
    ) -> Option<String> {
        use CompletedFragmentId::*;
        match (fragment, &self.expression) {
            (_, None) => None,
            (Function, Some(expr)) => Some(expr.func.repr()),
            (Argument { index }, Some(expr)) => {
                Some(expr.args.get(index)?.sast.wrapped.repr())
            }
        }
    }
    ...
    pub fn user_action(&self) -> UserAction {
        ...
        match self.next_completion_id() {
            CompletedFragmentId::Function if empty_pattern => {
                StartingTypingFunction
            }
            CompletedFragmentId::Function => TypingFunction,
            CompletedFragmentId::Argument { .. } if empty_pattern => {
                StartingTypingArgument
            }
    ...
    pub fn as_prefix_chain(
        &self,
        parser: &Parser,
    ) -> Option<ast::Shifted<ast::prefix::Chain>> {
        let parsed_pattern = parser.parse_line_ast(&self.pattern).ok();
        let pattern_sast =
            parsed_pattern.map(|p| ast::Shifted::new(self.pattern_offset, p));
...
impl ThisNode {
    ...
    pub fn new(
        ...
        let (var, needs_to_introduce_pattern) =
            if let Some(ast) = node.info.pattern() {
                // TODO [mwu]
...
impl FragmentAddedByPickingSuggestion {
    ...
    fn code_to_insert(
        ...
        let generate_this =
            self.id != CompletedFragmentId::Function || this_node.is_none();
        self.picked_suggestion
            .code_to_insert(Some(current_module), generate_this)
    }
}
...
impl Data {
    ...
    fn new_with_edited_node(
        ...
        let current_module =
            graph.module.path().qualified_module_name(project_name);
        let input = ParsedInput::new_from_ast(edited_node.info.expression());
        let suggestions = default();
        let intended_method =
            edited_node.metadata.and_then(|md| md.intended_method);
        let initial_entry =
            intended_method.and_then(|m| database.lookup_method(m));
        let initial_fragment = initial_entry.and_then(|entry| {
            ...
            // should be resolved.
            fragment
                .is_still_unmodified(&input, &current_module)
                .and_option(Some(fragment))
        });
        let mut fragments_added_by_picking =
            Vec::<FragmentAddedByPickingSuggestion>::new();
        initial_fragment.for_each(|f| fragments_added_by_picking.push(f));
        Ok(Data {
            input,
            actions: suggestions,
            fragments_added_by_picking,
        })
    }
    ...
    fn find_picked_fragment(
        ...
        self.fragments_added_by_picking
            .iter()
...
impl Searcher {
    ...
    pub async fn new(
        ...
        let graph = controller::ExecutedGraph::new(
            &parent,
            project.clone_ref(),
            method,
        )
        .await?;
        Self::new_from_graph_controller(
            parent,
            ide,
            project,
            graph,
    ...
    pub fn new_from_graph_controller(
        ...
        let def_span = double_representation::module::definition_span(
            &module_ast,
            &def_id,
        )?;
        let position =
            TextLocation::convert_span(module_ast.repr(), &def_span).end;
        let this_arg =
            Rc::new(matches!(mode, Mode::NewNode { .. }).and_option_from(
                || ThisNode::new(selected_nodes, &graph.graph()),
            ));
        let ret = Self {
            logger,
            graph,
    ...
    pub fn set_input(&self, new_input: String) -> FallibleResult {
        ...
        if expression_changed {
            ...
            self.reload_list();
        } else if let Actions::Loaded { list } =
            self.data.borrow().actions.clone_ref()
        {
    ...
    fn code_to_insert(
        &self,
        fragment: &FragmentAddedByPickingSuggestion,
    ) -> CodeToInsert {
        fragment.code_to_insert(
            &self.module_qualified_name(),
            self.this_arg.as_ref(),
        )
    }
    ...
    pub fn use_suggestion(
        ...
        let picked_completion = FragmentAddedByPickingSuggestion {
            id,
            picked_suggestion,
        };
        ...
        match action {
            ...
            Action::Example(example) => match *self.mode {
                Mode::NewNode { position } => {
                    self.add_example(&example, position).map(Some)
                }
            ...
            Action::ProjectManagement(action) => {
                match self.ide.manage_projects() {
                    Ok(_) => {
                        ...
                        executor::global::spawn(async move {
                            ...
                            if let Err(err) = result.await {
                                error!(
                                    logger,
                                    "Error when creating new project: {err}"
                                );
                            }
                        });
                        Ok(None)
                    ...
                    Err(err) => Err(NotSupported {
                        action_label: Action::ProjectManagement(action)
                            .to_string(),
                        reason: err,
                    }
                    .into()),
    ...
    pub fn is_function_fragment_unmodified(&self) -> bool {
        ...
        with(self.data.borrow(), |data| {
            data.fragments_added_by_picking.first().contains_if(|frag| {
                ...
                is_function
                    && frag.is_still_unmodified(&data.input, &current_module)
            })
        })
    }
    ...
    pub fn commit_node(&self) -> FallibleResult<ast::Id> {
        let _transaction_guard =
            self.graph.get_or_open_transaction("Commit node");
        let expr_and_method = || {
            let input_chain =
                self.data.borrow().input.as_prefix_chain(self.ide.parser());

            let expression = match (self.this_var(), input_chain) {
                (Some(this_var), Some(input)) => {
                    apply_this_argument(this_var, &input.wrapped.into_ast())
                        .repr()
                }
    ...
    pub fn add_example(
        ...
        let graph = self.graph.graph();
        let mut module = double_representation::module::Info {
            ast: graph.module.ast(),
        };
        let graph_definition_name = graph.graph_info()?.source.name.item;
        let new_definition =
            example.definition_to_add(&module, self.ide.parser())?;
        let new_definition_name =
            Ast::var(new_definition.name.name.item.clone());
        let new_definition_place =
            double_representation::module::Placement::Before(
                graph_definition_name,
            );
        module.add_method(
            new_definition,
            new_definition_place,
            self.ide.parser(),
        )?;
        ...
        let node_expression =
            ast::prefix::Chain::new_with_this(new_definition_name, here, args);
        let node_expression = node_expression.into_ast();
        let node = NodeInfo::from_main_line_ast(&node_expression)
            .ok_or(FailedToCreateNode)?;
        let added_node_id = node.id();
        let graph_definition = double_representation::module::locate(
            &module.ast,
            &self.graph.graph().id,
        )?;
        ...
        module.ast = module
            .ast
            .set_traversing(&graph_definition.crumbs, graph_info.ast())?;
        let metadata = NodeMetadata {
            position,
            ..default()
        };
        ...
        for import in example
            .imports
            .iter()
            .map(QualifiedName::from_text)
    ...
    fn invalidate_fragments_added_by_picking(&self) {
        ...
        data.fragments_added_by_picking.drain_filter(|frag| {
            !frag.is_still_unmodified(input, &current_module)
        });
    }
    ...
    fn add_required_imports(&self) -> FallibleResult {
        ...
        let imports = fragments
            .map(|frag| self.code_to_insert(frag).imports)
            .flatten();
        let mut module = self.module();
        let here = self.module_qualified_name();
        // TODO[ao] this is a temporary workaround. See [`Searcher::add_enso_project_entries`]
        //     documentation.
        let without_enso_project =
            imports.filter(|i| i.to_string() != ENSO_PROJECT_SPECIAL_MODULE);
    ...
    fn this_arg_type_for_next_completion(
        &self,
    ...
    fn return_types_for_argument_completion(
        &self,
        arg_index: usize,
    ) -> Vec<String> {
        let suggestions =
            if let Some(intended) = self.intended_function_suggestion() {
        ...
        suggestions
            ...
            .filter_map(|suggestion| {
                let arg_index =
                    arg_index + if self.this_arg.is_some() { 1 } else { 0 };
                let arg_index =
                    arg_index + if self.has_this_argument() { 1 } else { 0 };
                let parameter =
                    suggestion.argument_types().into_iter().nth(arg_index)?;
                Some(parameter)
            })
            .collect()
    }
    ...
    fn gather_actions_from_engine(
        ...
        executor::global::spawn(async move {
            ...
            let new_list =
                match this.make_action_list(responses, this_type, return_types)
                {
                    Ok(list) => Actions::Loaded {
                        list: Rc::new(list),
                    },
    ...
    fn make_action_list(
        ...
        completion_responses: Vec<
            json_rpc::Result<language_server::response::Completion>,
        >,
        _this_type: Option<String>,
        _return_types: Vec<String>,
    ) -> FallibleResult<action::List> {
        let creating_new_node =
            matches!(self.mode.deref(), Mode::NewNode { .. });
        let should_add_additional_entries =
            creating_new_node && self.this_arg.is_none();
        let mut actions = action::ListWithSearchResultBuilder::new();
        let (libraries_icon, default_icon) = action::hardcoded::ICONS
            .with(|i| (i.libraries.clone_ref(), i.default.clone_ref()));
        ...
        if should_add_additional_entries && self.ide.manage_projects().is_ok() {
            let mut root_cat =
                actions.add_root_category("Projects", default_icon.clone_ref());
            let category =
                root_cat.add_category("Projects", default_icon.clone_ref());
        ...
        if should_add_additional_entries {
            let examples_cat = libraries_root_cat
                .add_category("Examples", default_icon.clone_ref());
            examples_cat
                .extend(self.database.iterate_examples().map(Action::Example));
        }
        let libraries_cat = libraries_root_cat
            .add_category("Libraries", libraries_icon.clone_ref());
        if should_add_additional_entries {
        ...
        for response in completion_responses {
            ...
            let entries = response.results.iter().filter_map(|id| {
                self.database
                    .lookup(*id)
                    .map(|entry| {
                        Action::Suggestion(action::Suggestion::FromDatabase(
                            entry,
                        ))
                    })
                    .handle_err(|e| {
        ...
        let opt_result = || {
            ...
            if let Some(module) = self.module_whose_method_is_called(&call) {
                ...
                Some(
                    entry
                        .into_iter()
                        .map(action::Suggestion::FromDatabase)
                        .collect(),
                )
            } else {
                let name = &call.function_name;
                ...
                let entries = self
                    .database
                    .lookup_by_name_and_location(name, &module, location);
                Some(
                    entries
                        .into_iter()
                        .map(action::Suggestion::FromDatabase)
    ...
    fn module_whose_method_is_called(
        &self,
        call: &SimpleFunctionCall,
        ...
        not_local_name.and_option_from(|| {
            if this_name == constants::keywords::HERE
    ...
    fn module(&self) -> double_representation::module::Info {
        double_representation::module::Info {
            ast: self.graph.graph().module.ast(),
        }
    }
    ...
    fn module_qualified_name(&self) -> QualifiedName {
        ...
        self.graph
            ...
            .module
            .path()
            .qualified_module_name(project_name)
    }
    ...
    fn add_enso_project_entries(
        libraries_cat_builder: &action::CategoryBuilder,
    ) -> FallibleResult {
        for method in &["data", "root"] {
            ...
            let action = Action::Suggestion(action::Suggestion::FromDatabase(
                Rc::new(entry),
            ));
    ...
    fn add_hardcoded_entries(
        ...
        let this_type =
            this_type.map(tp::QualifiedName::from_text).transpose()?;
        let rt_converted =
            return_types.iter().map(tp::QualifiedName::from_text);
        let rt_result: FallibleResult<HashSet<tp::QualifiedName>> =
            rt_converted.collect();
        ...
        action::hardcoded::add_hardcoded_entries_to_list(
            list,
            this_type.as_ref(),
            return_types,
        );
...
fn apply_this_argument(this_var: &str, ast: &Ast) -> Ast {
    if let Ok(opr) = ast::known::Opr::try_from(ast) {
        ...
        if let Some(ref mut larg) = &mut infix.left {
            ...
            infix.left = Some(ast::opr::ArgWithOffset {
                arg: Ast::var(this_var),
                offset: 1,
            });
        }
...
pub mod test {
    ...
    impl Fixture {
        ...
        where
            F: FnOnce(&mut MockData, &mut language_server::MockClient),
        {
            ...
            let end_of_code =
                TextLocation::at_document_end(&data.graph.module.code);
            ...
            project
                ...
                .returning_st(move || project_qname.clone());
            project
                .expect_name()
                .returning_st(move || project_name.clone());
            let project = Rc::new(project);
            ide.expect_parser().return_const(Parser::new_or_panic());
            ide.expect_current_project()
                .returning_st(move || project.clone_ref());
            ide.expect_manage_projects().returning_st(move || {
                Err(ProjectOperationsNotSupported.into())
            });
            let module_name = QualifiedName::from_segments(
                data.graph.graph.project_name.clone(),
                &[MODULE_NAME],
            )
            .unwrap();
            let searcher = Searcher {
                ...
                data: default(),
                notifier: default(),
                mode: Immutable(Mode::NewNode {
                    position: default(),
                }),
                language_server: language_server::Connection::new_mock_rc(
                    client,
                ),
            ...
            let entry4 = model::suggestion_database::Entry {
                self_type: Some(
                    "test.Test.Test".to_owned().try_into().unwrap(),
                ),
    ...
    fn loading_list_w_self() {
        ...
        for case in &cases {
            let Fixture {
                ...
                data.expect_completion(
                    client,
                    case.sets_this.as_some(mock_type),
                    None,
                    &[1, 5, 9],
                );
            ...
            if case.sets_this {
                ...
                let this_node_id =
                    searcher.this_arg.deref().as_ref().unwrap().id;
            ...
            assert!(!searcher.actions().is_loading());
            searcher
                .use_suggestion(action::Suggestion::FromDatabase(
                    entry9.clone_ref(),
                ))
                .unwrap();
            searcher
    ...
    fn arguments_suggestions_for_picked_function() {
        ...
        searcher
            .set_input("testFunction2 'foo' ".to_owned())
            .unwrap();
        searcher
            .set_input("testFunction2 'foo' 10 ".to_owned())
            .unwrap();
    }
    ...
    fn non_picked_function_arg_suggestions() {
        let mut fixture = Fixture::new_custom(|data, client| {
            data.graph
                .module
                .code
                .insert_str(0, "import test.Test.Test\n\n");
        ...
        searcher
            .set_input(iformat!("{MODULE_NAME}.testMethod1 "))
            .unwrap();
        searcher
            .set_input("testFunction2 \"str\" ".to_string())
            .unwrap();
    ...
    fn non_picked_function_arg_suggestion_ambiguous() {
        fn run_case(input: impl Str, setup: impl FnOnce(&mut Fixture)) {
            ...
            let requested_types: Rc<RefCell<HashSet<Option<String>>>> =
                default();
            ...
            assert!(requested_types
                .borrow()
                .contains(&Some("Number".to_string())));
            assert!(requested_types
                .borrow()
                .contains(&Some("String".to_string())));
        }
    ...
    fn loading_list() {
        ...
        assert_eq!(
            list[2],
            Action::Suggestion(action::Suggestion::FromDatabase(entry1))
        );
        assert_eq!(
            list[3],
            Action::Suggestion(action::Suggestion::FromDatabase(entry9))
        );
    ...
    fn picked_completions_list_maintaining() {
        ...
        let frags_borrow = || {
            Ref::map(searcher.data.borrow(), |d| &d.fragments_added_by_picking)
        };
        ...
        searcher
            .set_input("testFunction1 some_arg pat".to_string())
            .unwrap();
        let (func,) = frags_borrow().iter().cloned().expect_tuple();
        assert_eq!(func.id, CompletedFragmentId::Function);
        assert!(are_same(&func.picked_suggestion, &entry1));
        ...
        let (function, arg1, arg2) =
            frags_borrow().iter().cloned().expect_tuple();
        ...
        searcher
            .set_input("testFunction1 some_arg TestVar1 TestV".to_string())
            .unwrap();
        ...
        searcher
            .set_input("testFunction2 some_arg TestVar1 TestV".to_string())
            .unwrap();
    ...
    fn applying_this_var() {
        ...
        let cases = [
            ...
            Case::new("+ bar baz", "foo + bar baz"),
            Case::new(
                "map x-> x.characters.length",
                "foo.map x-> x.characters.length",
            ),
    ...
    fn adding_node_introducing_this_var() {
            ...
            Case::new(
                ...
                |f| {
                    f.searcher
                        ...
                        .unwrap();
                    let new_parsed_input = ParsedInput::new(
                        "var.testFunction1",
                        &f.searcher.ide.parser(),
                    );
                    f.searcher.data.borrow_mut().input =
                        new_parsed_input.unwrap();
                },
            ),
        ...
        for case in cases.into_iter() {
            ...
            let updated_def =
                fixture.searcher.graph.graph().definition().unwrap().item;
    ...
    fn adding_imports_with_nodes() {
        fn expect_inserted_import_for(
            ...
            let picked_method = FragmentAddedByPickingSuggestion {
                id: CompletedFragmentId::Function,
                picked_suggestion: action::Suggestion::FromDatabase(
                    entry.clone_ref(),
                ),
            };
            with(searcher.data.borrow_mut(), |mut data| {
                ...
                data.input =
                    ParsedInput::new(entry.name.to_string(), &parser).unwrap();
            });
            ...
            let expected_import =
                expected_import.into_iter().cloned().collect_vec();
    ...
    fn committing_node() {
        ...
        with(searcher.data.borrow_mut(), |mut data| {
            data.fragments_added_by_picking.push(picked_method);
            data.input =
                ParsedInput::new("Test.testMethod1".to_string(), &parser)
                    .unwrap();
        });
        ...
        let (node1, node2) =
            searcher.graph.graph().nodes().unwrap().expect_tuple();
        ...
        assert_eq!(
            node2.metadata.unwrap().intended_method,
            expected_intended_method
        );

        // Edit existing node.
        searcher.mode = Immutable(Mode::EditNode {
            node_id: node1.info.id(),
        });
        searcher.commit_node().unwrap();
        let expected_code = "import test.Test.Test\nmain = \n    Test.testMethod1\n    operator1 = Test.testMethod1";
        let (node1, _) = searcher.graph.graph().nodes().unwrap().expect_tuple();
        assert_eq!(
            node1.metadata.unwrap().intended_method,
            expected_intended_method
        );
        assert_eq!(module.ast().repr(), expected_code);
    }
    ...
    fn initialized_data_when_editing_node() {
        ...
        let searcher_data = Data::new_with_edited_node(
        ...
        graph
            .set_expression(node_id, "Test.testMethod1 12")
            .unwrap();
        ...
        let (initial_fragment,) =
            searcher_data.fragments_added_by_picking.expect_tuple();
