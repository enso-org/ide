...
#[fail(
    display = "Wrong checksum of uploaded file: {}, local checksum is {}.",
    remote, local
)]
...
impl<DP: DataProvider> FileUploadProcess<DP> {
    ...
    pub async fn upload_chunk(&mut self) -> FallibleResult<UploadingState> {
        match self.file.data.next_chunk().await {
            Ok(Some(data)) => {
                ...
                self.bin_connection
                    .write_bytes(&self.remote_path, offset, false, &data)
                    .await?;
    ...
    async fn check_checksum(&mut self) -> FallibleResult {
        let remote = self
            .json_connection
            .file_checksum(&self.remote_path)
            .await?
            .checksum;
...
impl NodeFromDroppedFileHandler {
    ...
    pub fn create_node_and_start_uploading(
        ...
        executor::global::spawn(async move {
            if let Err(err) = this.upload_file(node, file).await {
                ...
                this.update_metadata(node, |md| {
                    md.error = Some(err.to_string())
                });
            }
        });
        Ok(())
    ...
    async fn upload_file(
        ...
        self.update_metadata(node, |md| {
            md.remote_name = Some(remote_name.clone())
        });
        self.update_expression(
            node,
            Self::uploading_node_expression(&remote_name),
        )?;
        ...
        while process.upload_chunk().await? == UploadingState::NotFinished {
            self.update_metadata(node, |md| {
                md.bytes_uploaded = process.bytes_uploaded
            });
        }
        self.update_expression(
            node,
            Self::uploaded_node_expression(&remote_name),
        )?;
        if let Err(err) = self
            .graph
            .module
            .with_node_metadata(node, Box::new(|md| md.uploading_file = None))
        {
    ...
    fn update_metadata(
        &self,
        node: ast::Id,
        f: impl FnOnce(&mut UploadingFile),
    ) {
        //TODO[ao] see the TODO comment in update_expression.
        let _tr = self
            .undo_redo_repository()
            .open_ignored_transaction("Upload Metadata Update");
    ...
    fn update_expression(
        ...
        let _tr = self
            .undo_redo_repository()
            .open_transaction("Update Uploading Node's Expression");
        self.graph.set_expression(node, expression)
    }

    async fn establish_remote_file_name(
        &self,
        original_name: &str,
    ) -> FallibleResult<String> {
        pick_non_colliding_name(
            &*self.project.json_rpc(),
            &self.data_path(),
            original_name,
        )
        .await
    ...
    fn uploading_node_expression(name: &str) -> String {
        format!(
            "File_Uploading.file_uploading Enso_Project.data/\"{}\"",
            name
        )
    }
...
mod test {
    ...
    type TestAsyncProvider =
        futures::channel::mpsc::Receiver<FallibleResult<Vec<u8>>>;
    type TestAsyncProviderSink =
        futures::channel::mpsc::Sender<FallibleResult<Vec<u8>>>;
    ...
    impl TestData {
        fn new_with_file_name(
            ...
            let path =
                Path::new(mock::data::ROOT_ID, &[DATA_DIR_NAME, &remote_name]);
            Self {
                chunks,
                file_size,
                file_name,
        ...
        fn setup_uploading_expectations(
            ...
            for chunk in self.chunks.iter().cloned() {
                ...
                binary_client
                    .expect_write_bytes()
                    .withf(move |p, off, ow, ch| {
                        *p == path && ch == chunk && *off == offset && !ow
                    })
                    .times(1)
                    .in_sequence(&mut write_seq)
                    .returning(move |_, _, _, _| {
                        future::ready(Ok(checksum.clone())).boxed_local()
                    });
                offset += chunk_len as u64;
            }
            let checksum = self.checksum.clone();
            json_client.expect.file_checksum(
                enclose!((self.path => path) move |p| {
                    assert_eq!(*p,path);
        ...
        fn file_to_upload_async(
            &self,
        ) -> (FileToUpload<TestAsyncProvider>, TestAsyncProviderSink) {
    ...
    impl UploadingFixture {
        fn new(logger: impl AnyLogger, data: TestData) -> Self {
            ...
            let json_con =
                Rc::new(language_server::Connection::new_mock(json_cli));

            Self {
                test: TestWithLocalPoolExecutor::set_up(),
                chunks: data.chunks.into_iter(),
                process: FileUploadProcess::new(
                    logger, file, bin_con, json_con, data.path,
                ),
    ...
    fn uploading_file() {
        ...
        let data =
            TestData::new(vec![vec![1, 2, 3, 4, 5], vec![3, 4, 5, 6, 7, 8]]);
        let mut test = UploadingFixture::new(logger, data);

        assert_eq!(
            test.next_chunk_result().unwrap(),
            UploadingState::NotFinished
        );
        assert_eq!(
            test.next_chunk_result().unwrap(),
            UploadingState::NotFinished
    ...
    fn creating_node_from_dropped_file() {
        ...
        let mut fixture = mock::Unified::new().fixture_customize(
            |_, json_rpc, binary_rpc| {
                json_rpc.expect.file_info(|path| {
                    assert_eq!(*path, data_path());
                    Ok(response::FileInfo {
                        attributes: data_dir_attributes(),
                    })
                });
                json_rpc.expect.file_list(|path| {
                    assert_eq!(*path, data_path());
                    let other_file = FileSystemObject::File {
                        name: "other".to_owned(),
                        path: data_path(),
                    };
        ...
        let handler = NodeFromDroppedFileHandler::new(
            logger,
            fixture.project,
            fixture.graph,
        );
        ...
        handler
            .create_node_and_start_uploading(file, position)
            .unwrap();
        assert_eq!(
            fixture.module.ast().repr(),
            module_code_uploading(TEST_FILE)
        );
        fixture.executor.run_until_stalled();
        assert_eq!(
            fixture.module.ast().repr(),
            module_code_uploaded(TEST_FILE)
        );
    }
    ...
    fn recreating_data_directory() {
        ...
        fixture
            .executor
            .expect_completion(handler.ensure_data_directory_exists())
            .unwrap();
    }
    ...
    fn name_collisions_are_avoided() {
        ...
        impl Case {
            ...
            fn run(self) {
                let Case {
                    file_name,
                    other_collision_name,
                    expected_remote_name,
                } = self;

                let logger =
                    Logger::new("test::creating_node_from_dropped_file");
                ...
                let mut fixture = mock::Unified::new().fixture_customize(
                    |_, json_rpc, binary_rpc| {
                        ...
                        json_rpc.expect.file_list(|path| {
                            ...
                            let file_2 = File {
                                name: other_collision_name,
                                path: data_path(),
                            };
                            Ok(response::FileList {
                                paths: vec![file_1, file_2],
                            })
                        });
                        data.setup_uploading_expectations(json_rpc, binary_rpc);
                    },
                ...
                let handler =
                    NodeFromDroppedFileHandler::new(logger, project, graph);
                let position = model::module::Position::new(45.0, 70.0);
                ...
                assert_eq!(
                    fixture.module.ast().repr(),
                    module_code_uploading(&file_name)
                );
                fixture.executor.run_until_stalled();
                assert_eq!(
                    fixture.module.ast().repr(),
        ...
        let with_extension =
            Case::new("text.file.txt", "text.file_1.txt", "text.file_2.txt");
        let starts_with_dot =
            Case::new(".gitignore", ".gitignore_1", ".gitignore_2");
    ...
    fn data_dir_attributes() -> FileAttributes {
        let dummy_time =
            UTCDateTime::parse_from_rfc3339("1996-12-19T16:39:57-08:00")
                .unwrap();
        FileAttributes {
