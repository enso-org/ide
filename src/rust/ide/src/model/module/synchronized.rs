...
impl ParsedContentSummary {
    ...
    fn from_source(source: &SourceFile) -> Self {
        ParsedContentSummary {
            ...
            code: TextLocation::convert_byte_range(
                &source.content,
                &source.code,
            ),
            id_map: TextLocation::convert_byte_range(
                &source.content,
                &source.id_map,
            ),
            metadata: TextLocation::convert_byte_range(
                &source.content,
                &source.metadata,
            ),
        }
...
impl Module {
    ...
    pub async fn open(
        ...
        let model = model::module::Plain::new(
            &logger,
            path,
            source.ast,
            source.metadata,
        ...
        executor::global::spawn(Self::runner(
            this.clone_ref(),
            summary,
            first_invalidation,
        ));
...
impl Module {
    ...
    async fn runner(
        ...
        first_invalidation: impl Future<
            Output = FallibleResult<ParsedContentSummary>,
        >,
    ) {
        let first_invalidation = first_invalidation.await;
        let mut ls_content =
            self.new_ls_content_info(initial_ls_content, first_invalidation);
        ...
        loop {
            ...
            match (notification, this) {
                (Some(notification), Some(this)) => {
                    debug!(
                        this.logger,
                        "Processing a notification: {notification:?}"
                    );
                    let result = this
                        .handle_notification(&ls_content, notification)
                        .await;
                    ls_content = this.new_ls_content_info(
                        ls_content.summary().clone(),
                        result,
                    )
                }
                _ => break,
    ...
    fn edit_for_snipped(
        ...
        (source != target).as_some_from(|| {
            let edit =
                TextEdit::from_prefix_postfix_differences(source, target);
    ...
    ) -> impl Future<Output = FallibleResult<ParsedContentSummary>> + 'static
    {
        debug!(
            self.logger,
            "Handling partial invalidation: {ls_content:?}."
        );
        let edits = vec![
    ...
    ) -> impl Future<Output = FallibleResult<ParsedContentSummary>> + 'static
    {
        ...
        let ls_future_reply =
            self.language_server.client.apply_text_file_edit(&edit);
        async {
    ...
    fn drop(&mut self) {
        ...
        executor::global::spawn(async move {
            ...
            if let Err(err) = result {
                error!(
                    logger,
                    "Error when closing module file {file_path}: {err}"
                );
            }
        });
    }
...
pub mod test {
    ...
    impl LsClientSetup {
        fn new(
            ...
            let current_ls_version =
                Sha3_224::new(current_ls_content.as_bytes());
        ...
        fn expect_edit_w_metadata(
            ...
            self.expect_some_edit(client, move |edit| {
                if let [edit_idmap, edit_code] = edit.edits.as_slice() {
                    ...
                    let idmap_range = TextLocation::convert_byte_range(
                        &file_so_far.content,
                        &file_so_far.id_map,
                    );
    ...
    fn apply_edit(code: &str, edit: &TextEdit) -> String {
        ...
        data::text::TextChange::replace(start..end, edit.text.clone())
            .applied(code)
    }
    ...
    fn apply_edits(code: impl Into<String>, file_edit: &FileEdit) -> String {
        ...
        file_edit
            .edits
            .iter()
            .fold(initial, |content, edit| apply_edit(&content, edit))
    }
    ...
    fn handling_notifications() {
        ...
        let test = |runner: &mut Runner| {
            ...
            let edit_handler = Rc::new(LsClientSetup::new(
                &data.logger,
                module_path,
                initial_code,
            ));
            let mut fixture = data.fixture_customize(|data, client, _| {
                ...
                edit_handler.expect_some_edit(client, |edit| {
                    assert!(edit
                        .edits
                        .last()
                        .map_or(false, |edit| edit.text.contains("Test")));
                    Ok(())
                });
    ...
    fn handling_notification_after_failure() {
        ...
        let test = |runner: &mut Runner| {
            ...
            let (_module, controller) =
                fixture.synchronized_module_w_controller();
            runner.perhaps_run_until_stalled(&mut fixture);
            let change = TextChange {
                replaced: text::Index::new(20)..text::Index::new(24),
                inserted: "Test 2".to_string(),
