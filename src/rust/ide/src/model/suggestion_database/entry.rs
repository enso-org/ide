...
#[fail(
    display = "Argument index {} is invalid for suggestion entry named {}.",
    index, name
)]
...
#[derive(Debug, Fail, Clone)]
#[fail(
    display = "Entry named {} is described as method but does not have a `this` parameter.",
    _0
)]
...
impl Entry {
    ...
    where
        TypeName: PartialEq<tp::QualifiedName>,
    {
    ...
    pub fn code_to_insert(
        ...
        let code = match this_expr {
            Some(this_expr) => format!(
                "{}{}{}",
                this_expr,
                ast::opr::predefined::ACCESS,
                self.name
            ),
    ...
    pub fn is_visible_at(
        ...
        match &self.scope {
            ...
            Scope::InModule { range } => {
                self.module == *module && range.contains(&location)
            }
        }
    }
    ...
    pub fn is_regular_method(&self) -> bool {
        ...
        let not_extension = self
            .self_type
            .contains_if(|this| this.in_module(&self.module))
    ...
    pub fn qualified_name(&self) -> tp::QualifiedName {
        tp::QualifiedName::new_module_member(
            self.module.clone(),
            self.name.clone(),
        )
    }
...
impl Entry {
    ...
    pub fn from_ls_entry(
        entry: language_server::types::SuggestionEntry,
    ) -> FallibleResult<Self> {
        ...
        let this = match entry {
            ...
            Module {
                ...
                let module_name: module::QualifiedName =
                    module.clone().try_into()?;
                Self {
                    documentation_html,
    ...
    pub fn apply_modifications(
        ...
        let module = m
            .module
            .map(|f| f.try_map(module::QualifiedName::from_text))
            .transpose();
        let self_type = m
            .self_type
            .map(|f| f.try_map(tp::QualifiedName::from_text))
            .transpose();
        let other_update_results = [
            ...
            module.and_then(|m| {
                Entry::apply_field_update("module", &mut self.module, m)
            }),
            self_type.and_then(|s| {
                Entry::apply_opt_field_update(
                    "self_type",
                    &mut self.self_type,
                    s,
        ...
        let other_update_results =
            SmallVec::from_buf(other_update_results).into_iter();
        let other_update_errors =
            other_update_results.filter_map(|res| res.err());
        let arg_update_errors = m
            .arguments
            .into_iter()
            .flat_map(|arg| self.apply_arg_update(arg));
    ...
    fn apply_scope_update(
        &mut self,
        update: Option<
            FieldUpdate<language_server::types::SuggestionEntryScope>,
        >,
    ) -> FallibleResult {
...
impl TryFrom<&Entry> for language_server::MethodPointer {
    ...
    fn try_from(entry: &Entry) -> FallibleResult<Self> {
        (entry.kind == Kind::Method)
            .ok_or_else(|| NotAMethod(entry.name.clone()))?;
        let missing_this_err = || MissingThisOnMethod(entry.name.clone());
        let defined_on_type =
            entry.self_type.clone().ok_or_else(missing_this_err)?;
...
impl From<&Entry> for span_tree::generate::context::CalledMethodInfo {
    fn from(entry: &Entry) -> span_tree::generate::context::CalledMethodInfo {
        let parameters =
            entry.arguments.iter().map(to_span_tree_param).collect();
...
mod test {
    ...
    fn code_from_entry() {
        let main_module =
            module::QualifiedName::from_text("local.Project.Main").unwrap();
        let another_module =
            module::QualifiedName::from_text("local.Project.Another_Module")
                .unwrap();
        let atom = Entry {
            name: "Atom".to_owned(),
            kind: Kind::Atom,
        ...
        let method = Entry {
            ...
            kind: Kind::Method,
            self_type: Some(
                "std.Base.Main.Number".to_string().try_into().unwrap(),
            ),
            ..atom.clone()
        };
