...
#[fail(
    display = "Encountered an empty definition ID. It must contain at least one crumb."
)]
...
#[derive(Fail, Clone, Debug)]
#[fail(
    display = "The definition with crumbs {:?} is not a direct child of the module.",
    _0
)]
...
impl QualifiedName {
    ...
    pub fn from_text(text: impl AsRef<str>) -> FallibleResult<Self> {
        ...
        if let [namespace_name, project_name, id_segments @ ..] =
            segments.as_slice()
        {
    ...
    pub fn from_segments(
        ...
        let module_segments =
            module_segments.map(|segment| segment.as_ref().to_string());
    ...
    pub fn from_all_segments(
        ...
        let typed_project_name = match (namespace, project_name) {
            (Some(ns), Some(name)) => {
                project::QualifiedName::from_segments(ns, name)
            }
    ...
    pub fn segments(&self) -> impl Iterator<Item = &str> {
        self.project_name
            .segments()
            .chain(self.id.segments.iter().map(|seg| seg.as_ref()))
    }
...
impl From<&QualifiedName> for String {
    fn from(name: &QualifiedName) -> Self {
        ...
        name.project_name
            .segments()
            .chain(segments)
            .join(ast::opr::predefined::ACCESS)
    }
}
...
impl ImportInfo {
    ...
    pub fn from_target_str(name: impl AsRef<str>) -> Self {
        ...
        let target = if name.is_empty() {
            ...
            name.split(ast::opr::predefined::ACCESS)
                .map(Into::into)
                .collect()
        };
        ImportInfo { target }
    ...
    pub fn from_qualified_name(name: &QualifiedName) -> Self {
        let target =
            name.segments().map(|segment| segment.to_string()).collect();
        Self { target }
    }
...
impl Info {
    ...
    pub fn used_names(&self) -> Vec<LocatedName> {
        let usage = double_representation::alias_analysis::analyze_crumbable(
            self.ast.shape(),
        );
    ...
    pub fn enumerate_imports(
        &self,
    ) -> impl Iterator<Item = (ModuleCrumb, ImportInfo)> + '_ {
        let children = self.ast.shape().enumerate();
        children.filter_map(|(crumb, ast)| {
            Some((crumb, ImportInfo::from_ast(ast)?))
        })
    }
    ...
    pub fn add_line(&mut self, index: usize, ast: Option<Ast>) {
        ...
        self.ast
            .update_shape(|shape| shape.lines.insert(index, line))
    }
    ...
    pub fn remove_line(
        &mut self,
        index: usize,
    ) -> FallibleResult<BlockLine<Option<Ast>>> {
        self.ast.update_shape(|shape| {
            shape
                .lines
                .try_remove(index)
                .ok_or_else(|| LineIndexOutOfBounds.into())
        })
    }
    ...
    pub fn remove_import(&mut self, to_remove: &ImportInfo) -> FallibleResult {
        let lookup_result = self
            .enumerate_imports()
            .find(|(_, import)| import == to_remove);
        let (crumb, _) =
            lookup_result.ok_or_else(|| ImportNotFound(to_remove.clone()))?;
    ...
    pub fn add_import(
        ...
        let previous_import = self
            .enumerate_imports()
            .take_while(|(_, import)| to_add.target > import.target)
            .last();

        let index_to_place_at =
            previous_import.map_or(0, |(crumb, _)| crumb.line_index + 1);
    ...
    pub fn add_ast(&mut self, ast: Ast, location: Placement) -> FallibleResult {
        ...
        let mut index = match location {
            ...
            Placement::Before(next_def) => {
                locate_line_with(&self.ast, &next_def)?.line_index
            }
            Placement::After(next_def) => {
                locate_line_with(&self.ast, &next_def)?.line_index + 1
            }
        };
...
impl ChildDefinition {
    fn try_retrieving_crumb(
        child: &definition::ChildDefinition,
    ) -> Option<ModuleCrumb> {
...
impl TryFrom<definition::ChildDefinition> for ChildDefinition {
    ...
    fn try_from(
        value: definition::ChildDefinition,
    ) -> Result<Self, Self::Error> {
        Self::new(value)
    }
}
...
pub fn lookup_method(
    ...
    let qualified_typename =
        tp::QualifiedName::from_text(&method.defined_on_type)?;
    let accept_here_methods = module_name == &qualified_typename;
    let method_module_name = QualifiedName::try_from(method)?;
    let implicit_extension_allowed =
        method.defined_on_type == method_module_name.to_string();
    for child in ast.def_iter() {
        ...
        let type_matches = match child_name.extended_target.as_slice() {
            ...
            [typename] => {
                let explicit_type_matching =
                    typename.item == qualified_typename.name;
                let here_extension_matching =
                    typename.item == HERE && accept_here_methods;
...
mod tests {
    ...
    fn import_adding_and_removing() {
        ...
        info.remove_import(&ImportInfo::from_target_str("Foo.Bar.Baz"))
            .unwrap();
        info.expect_code("import Bar.Gar\nimport Gar.Bar");
        info.remove_import(&ImportInfo::from_target_str("Foo.Bar.Baz"))
            .unwrap_err();
        info.expect_code("import Bar.Gar\nimport Gar.Bar");
        info.remove_import(&ImportInfo::from_target_str("Gar.Bar"))
            .unwrap();
        info.expect_code("import Bar.Gar");
        info.remove_import(&ImportInfo::from_target_str("Bar.Gar"))
            .unwrap();
        info.expect_code("");
    ...
    fn implicit_method_resolution() {
        let parser = parser::Parser::new_or_panic();
        let module_name =
            QualifiedName::from_all_segments(&["local", "ProjectName", "Main"])
                .unwrap();
        let expect_find =
            |method: &MethodPointer, code, expected: &definition::Id| {
        ...
        let expect_not_found = |method: &MethodPointer, code| {
            let module = parser.parse_module(code, default()).unwrap();
            lookup_method(&module_name, &module, method)
        ...
        let id = definition::Id::new_single_crumb(DefinitionName::new_method(
            "Main", "foo",
        ));
...
main = here.method1 10"#;
        ...
        let module =
            Info::from(parser.parse_module(module, default()).unwrap());
        ...
        let repr_after_insertion = |location| {
            ...
            module
                .add_method(to_add.clone(), location, &parser)
                .unwrap();
            module.ast.repr()
        };
