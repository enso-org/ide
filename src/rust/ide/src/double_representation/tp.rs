...
impl QualifiedName {
    ...
    pub fn from_text(text: impl Str) -> FallibleResult<Self> {
        ...
        let namespace = all_segments.next().ok_or_else(|| EmptyName {
            source: text.clone(),
        })?;
        let project_name =
            all_segments.next().ok_or_else(|| NoNamespaceName {
                source: text.clone(),
            })?;
        let project_name =
            project::QualifiedName::from_segments(namespace, project_name)?;
        let name_str =
            all_segments.next_back().ok_or_else(|| NoModuleName {
                source: text.clone(),
            })?;
    ...
    pub fn in_module(&self, module: &module::QualifiedName) -> bool {
        self.project_name == module.project_name
            && &self.module_segments == module.id.segments()
    }
}
...
mod test {
    ...
    fn qualified_name_from_string() {
        let valid_case = |text: &str,
                          ns_name: &str,
                          project_name: &str,
                          segments: Vec<&str>,
                          name: &str| {
            let project_name =
                project::QualifiedName::from_segments(ns_name, project_name)
                    .unwrap();
