...
#[fail(
    display = "Invalid path received from File Browser Component: {}",
    path
)]
struct InvalidPath {
    path: String,
...
pub fn from_file_browser_path(
    path: &std::path::Path,
) -> FallibleResult<language_server::Path> {
    use std::path::Component::*;
    let mut iter = path.components();
    match (iter.next(), iter.next()) {
        (Some(RootDir), Some(Normal(root_id))) => {
            let root_id = root_id.to_string_lossy().parse()?;
            Ok(language_server::Path::new(
                root_id,
                iter.map(|s| s.as_os_str().to_string_lossy()),
            ))
        }
        _ => {
    ...
    fn request_entries(
        ...
        let entries = self.content_roots.iter().filter_map(|root| {
            ...
            let (name, folder_type) = match &**root {
                ...
                language_server::ContentRoot::FileSystemRoot {
                    path, ..
                } => Some((path.clone(), FolderType::Root)),
                language_server::ContentRoot::Home { .. } => {
                    Some(("Home".to_owned(), FolderType::Home))
                }
                language_server::ContentRoot::Library { .. } => None, // We skip libraries, as they cannot be easily inserted.
                language_server::ContentRoot::Custom { .. } => None, // Custom content roots are not used.
            }?;
            let type_ = EntryType::Folder {
                type_: folder_type,
                content: {
                    let connection = self.connection.clone_ref();
                    DirectoryView::new_from_root(connection, root.clone_ref())
                        .into()
                },
            };
    ...
    fn request_entries(
        ...
        executor::global::spawn(async move {
            match this.get_entries_list().await {
                ...
                Err(error) => {
                    error_occurred.emit(ImString::new(error.to_string()))
                }
            }
        });
    }
...
pub fn create_node_from_file(
    ...
    let path = match &*content_root {
        ContentRoot::Project { .. } => {
            format!("Enso_Project.root/\"{}\"", path_segments)
        }
        ContentRoot::FileSystemRoot { path, .. } => {
            format!("\"{}/{}\"", path, path_segments)
        }
...
pub async fn do_file_operation(
    ...
    let src_name = ls_source.file_name().ok_or_else(|| InvalidSourceFile {
        path: source.to_string_lossy().to_string(),
    })?;
    let dest_name =
        pick_non_colliding_name(&*json_rpc, &ls_dest, src_name).await?;
    let dest_full = ls_dest.append_im(dest_name);
    match operation {
        FileOperation::Copy => {
            json_rpc.copy_file(&ls_source, &dest_full).await?
        }
        FileOperation::Move => {
            json_rpc.move_file(&ls_source, &dest_full).await?
        }
    }
    Ok(())
}
