...
impl Integration {
    ...
    , project       : model::Project
        ...
        frp::extend! { network
            eval_ editor_outs.node_editing_started([]{analytics::remote_log(analytics::AnonymousData("editor_node_editing_started"))});
            eval_ editor_outs.node_editing_finished([]{analytics::remote_log(analytics::AnonymousData("editor_node_editing_finished"))});
...
impl Model {
    ...
    pub fn on_node_entered(&self, local_call:&LocalCall) -> FallibleResult {
        analytics::remote_log(analytics::AnonymousData("integration_node_entered"));
    ...
    pub fn on_node_exited(&self, id:double_representation::node::Id) -> FallibleResult {
        analytics::remote_log(analytics::AnonymousData("integration_node_exited"));
        self.view.graph().frp.deselect_all_nodes.emit(&());
        self.request_detaching_all_visualizations();
        self.refresh_graph_view()?;
        ...
        if let Some(local_call) = local_call {
            ...
            let enter_action = async move {
                ...
                if let Err(e) = controller.enter_method_pointer(&local_call).await {
                    ...
                    let event = analytics::AnonymousData("integration_entering_node_failed");
    ...
    fn node_exited_in_ui(&self, _:&()) -> FallibleResult {
        ...
        let exit_node_action = async move {
            ...
            if let Err(e) = controller.exit_node().await {
                ...
                let event = analytics::AnonymousData("integration_exiting_node_failed");
                let data  = analytics::AnonymousData(|| format!("{:?}", e));
                analytics::remote_log_data(event, data)
            }
        };
