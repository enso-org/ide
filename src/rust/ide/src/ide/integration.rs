...
impl Integration {
    ...
    , project       : model::Project
        ...
        frp::extend! { network
            eval_ editor_outs.node_editing_started([]{analytics::remote_log("graph_editor::node_editing_started")});
            eval_ editor_outs.node_editing_finished([]{analytics::remote_log("graph_editor::node_editing_finished")});
            eval_ editor_outs.node_added([]{analytics::remote_log("graph_editor::node_added")});
            eval_ editor_outs.node_removed([]{analytics::remote_log("graph_editor::node_removed")});
            eval_ editor_outs.nodes_collapsed([]{analytics::remote_log("graph_editor::nodes_collapsed")});
            eval_ editor_outs.node_entered([]{analytics::remote_log("graph_editor::node_enter_request")});
            eval_ editor_outs.node_exited([]{analytics::remote_log("graph_editor::node_exit_request")});
            eval_ editor_outs.on_edge_endpoints_set([]{analytics::remote_log("graph_editor::edge_endpoints_set")});
            eval_ editor_outs.visualization_enabled([]{analytics::remote_log("graph_editor::visualization_enabled")});
            eval_ editor_outs.visualization_disabled([]{analytics::remote_log("graph_editor::visualization_disabled")});
            eval_ on_connection_removed([]{analytics::remote_log("graph_editor::connection_removed")});
            eval_ searcher_frp.used_as_suggestion([]{analytics::remote_log("searcher::used_as_suggestion")});
            eval_ project_frp.editing_committed([]{analytics::remote_log("project::editing_committed")});
        }
...
impl Model {
    ...
    pub fn on_node_entered(&self, local_call:&LocalCall) -> FallibleResult {
        analytics::remote_log("integration::node_entered");
    ...
    pub fn on_node_exited(&self, id:double_representation::node::Id) -> FallibleResult {
        analytics::remote_log("integration::node_exited");
        self.view.graph().frp.deselect_all_nodes.emit(&());
        self.request_detaching_all_visualizations();
        self.refresh_graph_view()?;
