...
pub fn main() {
    ...
    let _input_span_tree = builder::TreeBuilder::<node::Kind>::new(36)
    ...
    let input_span_tree2 = Node::<node::Kind>::new()
        .new_child(|t|t
            .new_ast_id()
            .kind(node::Kind::Chained)
            .crumbs(PrefixCrumb::Func)
            .new_child(|t|t.size(9).kind(node::Kind::Operation).crumbs(PrefixCrumb::Func).new_ast_id())
            .new_child(|t|t.size(1))
            .new_child(|t|t.size(4).kind(node::Kind::this().removable()).crumbs(PrefixCrumb::Arg).new_ast_id())
            .new_child(|t|t.size(1)))
        .new_child(|t|t
            .new_ast_id()
            ...
            .new_child(|t|t
                ...
                .kind(node::Kind::argument().removable())
                .crumbs(parens_cr)
                .new_child(|t|t.size(12).kind(node::Kind::Operation).crumbs(PrefixCrumb::Func).new_ast_id())
                .new_child(|t|t.size(1))
                .new_child(|t|t.size(6).kind(node::Kind::this().removable()).crumbs(PrefixCrumb::Arg).new_ast_id())
                .new_child(|t|t.size(1))
            ))
        .new_child(|t|t.size(1));
