...
pub fn main() {
    ...
    let _input_span_tree = builder::TreeBuilder::<()>::new(36)
        .add_child(0,14,Kind::Chained,PrefixCrumb::Func)
            ...
            .add_child(10,4,Kind::this(true),PrefixCrumb::Arg)
                .set_expression_id(Uuid::new_v4())
                .done()
            .add_empty_child(14,InsertionPointType::Append)
            .set_expression_id(Uuid::new_v4())
            .done()
        .add_child(15,21,Kind::argument(true,None,None),PrefixCrumb::Arg)
            .set_expression_id(Uuid::new_v4())
            .add_child(1,19,Kind::argument(false,None,None),parens_cr1)
                .set_expression_id(Uuid::new_v4())
                .add_child(0,12,Kind::Operation,PrefixCrumb::Func)
                    .set_expression_id(Uuid::new_v4())
                    .done()
                .add_empty_child(13,InsertionPointType::BeforeTarget)
                .add_child(13,6,Kind::this(false),PrefixCrumb::Arg)
                    .set_expression_id(Uuid::new_v4())
                    .done()
    ...
    let input_span_tree2 = Node::<()>::new()
        .new_child(|t|t
            ...
            .new_child(|t|t.size(4).kind(Kind::this(true)).crumbs(PrefixCrumb::Arg).new_id())
            .new_child(|t|t.size(1)))
        .new_child(|t|t
            .new_id()
            .kind(Kind::argument(true,None,None))
            ...
            .new_child(|t|t
                ...
                .new_child(|t|t.size(1))
                .new_child(|t|t.size(6).kind(Kind::this(true)).crumbs(PrefixCrumb::Arg).new_id())
                .new_child(|t|t.size(1))
            ))
        .new_child(|t|t.size(1));
