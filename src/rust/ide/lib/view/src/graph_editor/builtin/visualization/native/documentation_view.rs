//! Documentation View visualisation implemented using the native shape system.
...
pub mod shape {
    ...
    ensogl::define_shape_system! {
        (position:Vector2<f32>,radius:f32) {
            ...
            let node = node.fill(Rgba::new(0.33,0.46,0.99,1.0));
...
impl DocumentationView {
    ...
    pub fn new(scene:&Scene) -> Self {
        ...
        let signature      = Signature::new_for_any_type(Path::builtin("[Demo] Bubble Chart"),
                                                         Format::Json);
        let model = DocumentationViewModel{display_object,views,logger,size,scene,signature};

        DocumentationView {frp,network,model} . init()
    }
