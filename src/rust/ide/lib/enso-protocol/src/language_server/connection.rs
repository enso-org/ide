...
#[derive(Fail, Debug)]
#[fail(
    display = "Failed to initialize language server RPC connection: {}.",
    _0
)]
...
impl Connection {
    ...
    fn extract_project_root(
        content_roots: &mut Vec<ContentRoot>,
    ) -> FallibleResult<ContentRoot> {
        let opt_index = content_roots
            .iter()
            .position(|cr| matches!(cr, ContentRoot::Project { .. }));
