...
pub trait UuidExt {
    ...
    fn from_bytes_split(
        least_significant: [u8; 8],
        most_significant: [u8; 8],
    ) -> Self;
}
...
impl UuidExt for Uuid {
    ...
    fn from_bytes_split(
        ...
        let all_bytes = least_significant
            .iter()
            .chain(most_significant.iter())
            .rev();
...
mod tests {
    ...
    fn least_significant_bits() {
        let uuid =
            Uuid::parse_str("38400000-8cf0-11bd-b23e-10b96e4ef00d").unwrap();
    ...
    fn uuid_round_trips() {
        let uuid =
            Uuid::parse_str("6de39f7b-df3a-4a3c-84eb-5eaf96ddbac2").unwrap();
