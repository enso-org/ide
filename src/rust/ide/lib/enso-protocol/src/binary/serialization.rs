...
trait SerializableDeserializableObject<'a>: Sized {
    ...
    fn deserialize_required_opt(
        fbs: Option<Self::Out>,
    ) -> Result<Self, DeserializationError> {
        let missing_expected =
            || DeserializationError("Missing expected field".to_string());
...
impl<'a> SerializableDeserializableObject<'a>
    for message::VisualisationContext
{
...
impl<'a> SerializableDeserializableObject<'a> for LSPath {
    ...
    fn deserialize(fbs: Self::Out) -> Result<Self, DeserializationError> {
        let missing_root_id =
            || DeserializationError("Missing root ID".to_string());
    ...
    fn serialize(
        ...
        Some(match self {
            ...
            ToServerPayload::ReadFile { path } => {
                let path = path.serialize(builder);
                ReadFileCommand::create(
                    builder,
                    &ReadFileCommandArgs { path: Some(path) },
                )
                .as_union_value()
            }
            ToServerPayload::ReadBytes { segment } => {
                let segment = segment.serialize(builder);
                ReadBytesCommand::create(
                    builder,
...
impl SerializableUnion for ToServerPayloadOwned {
    ...
    fn serialize(
        ...
        Some(match self {
            ...
            ToServerPayloadOwned::WriteBytes {
                path,
                byte_offset,
                overwrite,
                bytes,
    ...
    fn active_variant(&self) -> Self::EnumType {
        match self {
            ToServerPayloadOwned::InitSession { .. } => {
                InboundPayload::INIT_SESSION_CMD
            }
            ToServerPayloadOwned::WriteFile { .. } => {
                InboundPayload::WRITE_FILE_CMD
            }
            ToServerPayloadOwned::ReadFile { .. } => {
                InboundPayload::READ_FILE_CMD
            }
            ToServerPayloadOwned::WriteBytes { .. } => {
                InboundPayload::WRITE_BYTES_CMD
            }
            ToServerPayloadOwned::ReadBytes { .. } => {
                InboundPayload::READ_BYTES_CMD
            }
            ToServerPayloadOwned::ChecksumBytes { .. } => {
                InboundPayload::CHECKSUM_BYTES_CMD
            }
        }
    }
...
impl SerializableUnion for FromServerPayloadOwned {
    ...
    fn active_variant(&self) -> Self::EnumType {
        match self {
            ...
            FromServerPayloadOwned::WriteBytesReply { .. } => {
                OutboundPayload::WRITE_BYTES_REPLY
            }
            FromServerPayloadOwned::ReadBytesReply { .. } => {
...
impl SerializableUnion for Option<message::ErrorPayload> {
    ...
    fn serialize(
        ...
        Some(match self.as_ref()? {
            message::ErrorPayload::ReadOOB { file_length } => {
                let args = ReadOutOfBoundsErrorArgs {
    ...
    fn active_variant(&self) -> Self::EnumType {
        match self {
            Some(message::ErrorPayload::ReadOOB { .. }) => {
...
impl<'a> DeserializableUnionField<'a, OutboundMessage<'a>>
    for FromServerPayload<'a>
{
    fn deserialize(
        ...
        match message.payload_type() {
            OutboundPayload::ERROR => {
                ...
                Ok(FromServerPayload::Error {
                    ...
                    data: Option::<message::ErrorPayload>::deserialize(
                        payload,
                    )?,
                })
            }
            ...
            OutboundPayload::VISUALISATION_UPDATE => {
                let payload =
                    message.payload_as_visualisation_update().unwrap();
                let context = payload.visualisationContext();
                Ok(FromServerPayload::VisualizationUpdate {
                    data: payload.data(),
                    context: message::VisualisationContext::deserialize(
                        context,
                    )?,
                })
            }
            OutboundPayload::WRITE_BYTES_REPLY => {
                ...
                Ok(FromServerPayload::WriteBytesReply {
                    checksum: message::EnsoDigest::deserialize(
                        payload.checksum(),
                    )?,
                })
            }
            ...
            OutboundPayload::CHECKSUM_BYTES_REPLY => {
                let payload =
                    message.payload_as_checksum_bytes_reply().unwrap();
                let checksum =
                    message::EnsoDigest::deserialize(payload.checksum())?;
...
impl<'a> DeserializableUnionField<'a, InboundMessage<'a>>
    for ToServerPayloadOwned
{
    fn deserialize(
        ...
        match message.payload_type() {
            ...
            InboundPayload::READ_BYTES_CMD => {
                ...
                Ok(ToServerPayloadOwned::ReadBytes {
                    segment: message::FileSegment::deserialize(
                        payload.segment(),
                    )?,
                })
            }
...
impl<'a> DeserializableUnionField<'a, Error<'a>>
    for Option<message::ErrorPayload>
{
    fn deserialize(owner: Error<'a>) -> Result<Self, DeserializationError> {
        match owner.data_type() {
            ...
            ErrorPayload::READ_OOB => {
                ...
                Ok(Some(message::ErrorPayload::ReadOOB {
...
where
    T: SerializableUnion<EnumType = InboundPayload>,
{
    fn write(&self, builder: &mut FlatBufferBuilder) {
        ...
        let message = InboundMessage::create(
            builder,
            &InboundMessageArgs {
...
where
    T: SerializableUnion<EnumType = OutboundPayload>,
{
    fn write(&self, builder: &mut FlatBufferBuilder) {
        let correlation_id = self.correlation_id.map(EnsoUUID::from);
        let message_id = self.message_id.into();
        let payload_type = self.payload.active_variant();
        let payload = self.payload.serialize(builder);
        let message = OutboundMessage::create(
            builder,
            &OutboundMessageArgs {
                correlationId: correlation_id.as_ref(),
                messageId: Some(&message_id),
...
where
    T: DeserializableUnionField<'a, InboundMessage<'a>>,
{
    fn deserialize(data: &'a [u8]) -> Result<Self, DeserializationError> {
...
where
    T: DeserializableUnionField<'a, OutboundMessage<'a>>,
{
    fn deserialize(data: &'a [u8]) -> Result<Self, DeserializationError> {
