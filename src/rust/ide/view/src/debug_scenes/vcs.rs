...
fn init(app:&Application) {
    ...
    let node4_id = graph_editor.add_node();
    ...
    graph_editor.frp.set_node_position.emit((node4_id,Vector2(250.0,300.0)));
    ...
    let expression_4 = expression_mock2();
    graph_editor.frp.set_node_expression.emit((node4_id,expression_4));

    graph_editor.frp.set_node_vcs_status.emit((node1_id,Some(vcs::Status::Edited)));
    graph_editor.frp.set_node_vcs_status.emit((node2_id,Some(vcs::Status::Added)));
    graph_editor.frp.set_node_vcs_status.emit((node3_id,Some(vcs::Status::Unchanged)));
    graph_editor.frp.set_node_vcs_status.emit((node4_id,None));
