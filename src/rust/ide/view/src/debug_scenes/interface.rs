...
fn init(app: &Application) {
    let _bg = app
        ...
        .scene()
        .style_sheet
        .var(theme::application::background);
    ...
    project_view
        .status_bar()
        .add_event(status_bar::event::Label::new("This is a status message."));
    ...
    graph_editor
        .frp
        .set_node_position
        .emit((node1_id, Vector2(-150.0, 50.0)));
    graph_editor
        .frp
        .set_node_position
        .emit((node2_id, Vector2(50.0, 50.0)));
    graph_editor
        .frp
        .set_node_position
        .emit((node3_id, Vector2(150.0, 250.0)));

    let expression_1 = expression_mock();
    graph_editor
        .frp
        .set_node_expression
        .emit((node1_id, expression_1.clone()));
    let comment_1 = String::from("Sample documentation comment.");
    graph_editor
        .frp
        .set_node_comment
        .emit((node1_id, comment_1));

    let expression_2 = expression_mock3();
    graph_editor
        .frp
        .set_node_expression
        .emit((node2_id, expression_2.clone()));

    let expression_3 = expression_mock2();
    graph_editor
        .frp
        .set_node_expression
        .emit((node3_id, expression_3));
    ...
    let error = graph_editor::component::node::Error {
        kind,
        message,
        propagated,
    };
    graph_editor
        .frp
        .set_node_error_status
        .emit((node3_id, Some(error)));

    let foo_node = graph_editor.add_node_below(node3_id);
    graph_editor
        .set_node_expression
        .emit((foo_node, Expression::new_plain("foo")));
    ...
    let min_spacing =
        styles.get_number(theme::graph_editor::minimal_x_spacing_for_new_nodes);
    let gap_between_nodes =
        styles.get_number(theme::graph_editor::default_x_gap_between_nodes);
    let gap_for_bar_node = min_spacing + gap_between_nodes + f32::EPSILON;
    graph_editor.set_node_position((
        baz_node,
        baz_position + Vector2(gap_for_bar_node, 0.0),
    ));

    let bar_node = graph_editor.add_node_below(node3_id);
    graph_editor
        .set_node_expression
        .emit((bar_node, Expression::new_plain("bar")));

    // === Connections ===

    let src = graph_editor::EdgeEndpoint::new(
        node1_id,
        span_tree::Crumbs::new(default()),
    );
    let tgt = graph_editor::EdgeEndpoint::new(
        node2_id,
        span_tree::Crumbs::new(vec![0, 0, 0, 0, 1]),
    );
    ...
    graph_editor
        .frp
        .set_node_position
        .emit((dummy_node_added_id, Vector2(-450.0, 50.0)));
    graph_editor
        .frp
        .set_node_position
        .emit((dummy_node_edited_id, Vector2(-450.0, 125.0)));
    graph_editor
        .frp
        .set_node_position
        .emit((dummy_node_unchanged_id, Vector2(-450.0, 200.0)));

    let dummy_node_added_expr = expression_mock_string("This node was added.");
    let dummy_node_edited_expr =
        expression_mock_string("This node was edited.");
    let dummy_node_unchanged_expr =
        expression_mock_string("This node was not changed.");

    graph_editor
        .frp
        .set_node_expression
        .emit((dummy_node_added_id, dummy_node_added_expr));
    graph_editor
        .frp
        .set_node_expression
        .emit((dummy_node_edited_id, dummy_node_edited_expr));
    graph_editor
        .frp
        .set_node_expression
        .emit((dummy_node_unchanged_id, dummy_node_unchanged_expr));

    graph_editor
        .frp
        .set_node_vcs_status
        .emit((dummy_node_added_id, Some(vcs::Status::Edited)));
    graph_editor
        .frp
        .set_node_vcs_status
        .emit((dummy_node_edited_id, Some(vcs::Status::Added)));
    graph_editor
        .frp
        .set_node_vcs_status
        ...
        .for_each(|node| {
            if let Some(expr_id) = node.ast_id {
                ...
                graph_editor
                    ...
                    .emit((node1_id, expr_id, dummy_type));
            }
        });
        ...
        .for_each(|node| {
            if let Some(expr_id) = node.ast_id {
                ...
                graph_editor
                    .frp
                    .set_expression_usage_type
                    .emit((node2_id, expr_id, dummy_type));
            }
        });
...
pub fn expression_mock_string(label: &str) -> Expression {
    ...
    let ctx = span_tree::generate::MockContext::new_single(
        ast.id.unwrap(),
        invocation_info,
    );
...
pub fn expression_mock2() -> Expression {
    ...
    let input_span_tree = span_tree::builder::TreeBuilder::new(36)
        ...
        .add_child(
            10,
            4,
            span_tree::node::Kind::this().removable(),
            PrefixCrumb::Arg,
        )
        .set_ast_id(Uuid::new_v4())
        .done()
        ...
        .add_child(
            15,
            21,
            span_tree::node::Kind::argument().removable(),
            PrefixCrumb::Arg,
        )
        .set_ast_id(Uuid::new_v4())
