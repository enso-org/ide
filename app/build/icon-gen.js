...
async function genIcons() {
    ...
    await fsp.mkdir('dist/icon/svg', {recursive:true})
    await fsp.mkdir('dist/icon/png', {recursive:true})
    for (let size of sizes) {
        let name = `icon_${size}x${size}.svg`
        await fsp.writeFile(`dist/icon/svg/${name}`,exports.generateMinimalWhiteLogo(size,true))
    }
    ...
    for (let size of sizes) {
        ...
        await sharp(`dist/icon/svg/${inName}`,{density:144}).png().resize({
            width  : size,
            kernel : sharp.kernel.mitchell
        }).toFile(`dist/icon/png/${outName}`)
    }
    ...
    exec(`cp -R dist/icon/png dist/icon/png.iconset`)
    exec(`iconutil --convert icns --output dist/icon/icon.icns dist/icon/png.iconset`)

    console.log("Generating ICO.")
    let files = []
    for (let size of win_sizes) {
        let inName = `icon_${size}x${size}.png`
        let data   = await fsp.readFile(`dist/icon/png/${inName}`)
        files.push(data)
    }
    toIco(files).then(buf => { fs.writeFileSync('dist/icon/icon.ico', buf) })
}
