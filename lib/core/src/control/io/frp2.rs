...
impl<T:Debug> KnownOutput for EmitterShape<T> {
    type Output = T;
}

impl<T> KnownInput for EmitterShape<T> {
    type Input = ();
}

impl<T:Debug> NodeOps for Emitter<T> {}



type Emitter<T> = NodeTemplate<EmitterShape<T>>;
