...
impl RenderedLines {
    ...
    pub fn jump_to(&mut self, scroll_position:Vector2<f64>) {
        self.scroll(scroll_position - self.window_offset)
    }
    ...
    -> RenderedLines {
        ...
        RenderedLines {
            window_size: display_size,max_chars_in_fragment,
    ...
    fn displayed_x_range(&self) -> RangeInclusive<f64> {
        let begin = self.window_offset.x;
        let end   = begin + self.window_size.x;
        begin..=end
    }

    fn displayed_lines(&self, lines_count:usize) -> RangeInclusive<usize> {
        let top                      = self.window_offset.y;
        let bottom                   = self.window_offset.y - self.window_size.y;
    ...
    fn setup_buffers(&mut self, gl_context:&Context, refresh:RefreshInfo) {
        ...
        let all_fragments   = 0..self.fragments.rendered_lines.len();
    ...
    -> FragmentsDataBuilder<'a> {
        let line_clip_left  = self.window_offset.x;
        let line_clip_right = line_clip_left + self.window_size.x;
    ...
    pub fn vertices_count(&self) -> usize {
        BASE_LAYOUT_SIZE * self.fragments.rendered_lines.len() * self.max_chars_in_fragment
    }
}
