...
mod tests {
	...
	fn usage() {
		...
		for _ in 0..51 {
	        ...
	        object.set_rotation(random() as f32 * TAU, random() as f32 * TAU, random() as f32 * TAU);
	...
	fn rhs_coordinates() {
		...
		for mask in vec![(1, 0, 0), (0, 1, 0), (0, 0, 1)] {
			for i in 0..10 {
				...
				object.element.style().set_property("background-color", &format!("rgba({}, {}, {}, {})", r as u8, g as u8, b as u8, 1.0)).expect("set background-color");
				scene.add(object);
			}
		}
